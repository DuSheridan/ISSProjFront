...
import { SectieService } from '../../services/sectie.service';
...
export class AngajatComponent implements OnInit {
  ...
  constructor(private sectie_service: SectieService, private router: Router) { }

  ngOnInit(): void {
    console.log(localStorage.getItem('isLogged'))
    console.log(localStorage.getItem('id'))
    console.log(localStorage.getItem('tip'))
    if (localStorage.getItem('isLogged') != 'true' || localStorage.getItem('tip') != 'Operator_Sectie'){
  ...
  getMedicament(){
    this.sectie_service.getMedicament().subscribe(result => this.medicamente = result.data);
  }
  ...
  inregistrareComanda(){
    ...
    var result_reg:any;
    this.sectie_service.inregistrareComanda(data).subscribe(result => result_reg=result);
    setTimeout(()=>{
      this.getMedicament()
      if (result_reg == "ok"){
  ...
  getComenzi(): void {
    ...
    this.sectie_service.getComenzi(operator_sectie_id).subscribe(comenzi => this.comenzi_active = comenzi.data);
  }
